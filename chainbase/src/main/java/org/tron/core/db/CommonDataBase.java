...
public class CommonDataBase extends TronDatabase<byte[]> {
  ...
  public void saveLatestPbftBlockHash(ByteString data) {
    this.put(LATEST_PBFT_BLOCK_HASH, data.toByteArray());
  }

  public ByteString getLatestPbftBlockHash() {
    byte[] date = this.get(LATEST_PBFT_BLOCK_HASH);
    return ByteString.copyFrom(date);
  }

  public Sha256Hash getLatestPbftBlockHashId() {
    ByteString latestBlockHash = getLatestPbftBlockHash();
    return Sha256Hash.of( latestBlockHash.toByteArray());
  }
